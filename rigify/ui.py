...
class DATA_OT_rigify_set_custom_color(bpy.types.Operator):
    bl_idname = "armature.rigify_set_custom_colors"
    bl_label = "Set custom colors preset"

    def itemsList(self, context):
        obj = context.object
        listItems = []
        for i, col in enumerate(obj.data.rigify_colors):
            listItems.append((obj.data.rigify_colors[i].name))
        print("## ITEMS: %s" % listItems)
        self.testing = listItems
        #return listItems
    ...
    def execute(self, context):
        ...
        data = []
        self.itemsList(context)
        for a, b in (self.as_keywords(ignore=("chooseSet", "presetName", "limitImport", "do_update", "overwrite"))).items():
        ...
        data = dict(data)
        #row.operator('rigify.exportdata').data = repr([repr(data), obj.presetName, obj.overwrite])
        print("### DATA: %s" % data)
        #bpy.ops.rigify.exportdata().data = repr([repr(data), obj.presetName, obj.overwrite])
        bpy.ops.rigify.exportdata("INVOKE_DEFAULT", data = repr([repr(data), obj.presetName, obj.overwrite]))
...
def register():
    ...
    bpy.utils.register_class(DATA_OT_rigify_set_custom_color)
...
def unregister():
    ...
    bpy.utils.unregister_class(DATA_OT_rigify_set_custom_color)
