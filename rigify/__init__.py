...
if "bpy" in locals():
    ...
    importlib.reload(add_rig_presets)
else:
    from . import utils, rig_lists, generate, ui, metarig_menu, add_rig_presets
...
def ThemeColors(self, context):
    Colors=[]
    theme = bpy.context.user_preferences.themes[0]
    for i, ui in enumerate(theme.bone_color_sets):
        theme_color_set = "%0.2d - Theme Color Set" % (i +1)
        theme = "THEME%0.2d" % (i +1)
        icon = 'COLORSET_'+'%0.2d' % (i +1)+'_VEC'
        Colors.append((theme,theme_color_set,theme_color_set, icon, i))
...
def register():
    ...
    bpy.types.Armature.rigify_theme_to_add = bpy.props.EnumProperty(items=ThemeColors, name='Theme')

    bpy.types.Armature.rigify_custom_colors_enable = bpy.props.BoolProperty(default=False)

    #bpy.types.Armature.rigify_custom_theme_add = bpy.props.EnumProperty(items=ThemeColors, name='Theme')
...
def unregister():
    ...
    del IDStore.rigify_presets
    del IDStore.rigify_presetName
    del IDStore.rigify_overwrite
